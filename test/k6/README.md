# k6 tests for local-backend-setup

## NOTE: 
This is pending guild level decisions of standards and best practices - seach Confluence for more details.
...
# Below is the content of the readme as per the Github template [grafana /
...
This also needs to be done before running a test following any amendments
...
```bash
$ k6 run dist/smokeTest/{TESTNAME}.js
```
...
For adding or removing tests, amend `module.exports > entry`:
```
module.exports = {
  ...
  entry: {
    ... // Add or remove test entries from here.
  }
  ...
}
```
