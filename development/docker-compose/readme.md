...
1. View a list of all the running containers, with their status and configuration:
    ...
    docker ps
    ```
2. To set the Docker image for the version of Edge you are running, replace `2025.09.42-LTS` with the value of `BB_VERSION` in the [development/docker-compose/.env](https://github.com/backbase/local-backend-setup/blob/main/development/docker-compose/.env) file.:
    ```shell
    docker pull repo.backbase.com/backbase-docker-releases/edge:2025.09.42-LTS
    ```
