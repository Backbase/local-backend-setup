...
public class KafkaErrorTopicChecker {
    ...
    public boolean areMessagesOnErrorTopic(String errorTopic, int partitionId) {
        try (Consumer<String, String> consumer = consumerFactory.createConsumer()) {
            ...
            ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(rchKafkaGenericProperties.getPollDuration()));
            log.debug("Error message count on error topic:{}:" , records.count());
